...
Cloud execution is a convenient extension to the [local/on-premise execution]({{ site.baseurl }}{% link _nextgen/test-running/local-on-premise-execution.md %}) capability of k6. Great when you need to run larger tests or distribute traffic generation across several geographic locations (aka "load zones").
...
When running a k6 test in the cloud two [tags]({{ site.baseurl }}{% link _nextgen/test-scripting/tags.md %}) are added to all metrics:
...
![Insights tags]({{ site.baseurl }}/assets/img/nextgen/test-running/cloud-execution-tags.png)
